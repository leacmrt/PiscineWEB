...
$sql0 = "SELECT ID FROM items WHERE ((Nom = \"".$_SESSION['NomArticle']."\")  AND (ID_vendeur = \"".$_SESSION['ID_vendeur']."\"))";
$result0 = mysqli_query($db_handle, $sql0);
$ID = mysqli_fetch_row($result0)[0];
    ...
    <div style=" border :1px solid  ; border-color:black ; width: 600px; height: 460px">
      <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
       ...
       <br><br><br>
        ...
        if($_SESSION['Meilleure'])
        {
          $sqlRecherche = "SELECT * FROM offre WHERE ((ID_Acheteur = ".$VA.") AND (ID_Item = ".$ID."))";
          ...
          if ($Echange = mysqli_fetch_assoc($resultRecherche))
          {
            ...
            {
              ?> <center>
              <form action="Montant_Meilleur2.php" method="post"> 
              <input type="hidden" name="ID_Offre" value=<?php echo $Echange['ID_Offre']?> >
              <input type="submit" name="button" value="Une offre !" style="background-color: #ea8685">
              </form> </center>
              <?php
            }
          }
